findSuspectsKNN <- function(
  ...
  #r <- scale(log(10000 * assays(x)$count / Matrix::rowSums(assays(x)$depth) + 1)) # second scaling method
  #r <- scale(Normalize(assays(x)$count/(assays(x)$depth+0.01))) #third scaling method
  ...
  is_suspect <- n_pseudo_neighbor > knn_threshold & !rowData(x)$is_pseudo # more neighbors were pseudo than real data
  if (n_samples < 100) {
